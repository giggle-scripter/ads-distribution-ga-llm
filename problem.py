...
def random_generate(num_billboards: int, num_ads: int) -> Problem:
    ...
    This function creates a random problem with:
    - Random billboard configurations (1-6 sides each)
    - Random ad base prices (40-200 range)
    - Random conflict relationships (40% probability between any two ads)
    ...
    for _ in range(num_ads):
        # Random price between 40 and 200
        price = random.randint(200, 500)
        ads_base_price.append(price)
        ads_max_budget.append(int(price * random.uniform(1.11, 2.02)))
