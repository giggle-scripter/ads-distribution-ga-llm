"""
Exact Algorithm Module - Backtracking with Branch and Bound

This module implements an exact algorithm for the advertising distribution problem
using backtracking with branch and bound optimization. The algorithm explores
all possible assignments while pruning branches that cannot lead to optimal solutions.

Key Features:
- Complete exploration of solution space
...
def branch_and_bound(problem: Problem, time_limit: float = 300.0) -> Tuple[List[int], dict]:
    """
    Solve the advertising distribution problem using exact algorithm.
